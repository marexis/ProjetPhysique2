...
def composant_CD4511(entree):
    ...
    while i < 4:
        #print (entree[7-i])
        lettre += entree[7-i]*(2**(3-i))
        i+=1
    #print("fin boucle")
    if lettre == 0:#' '
        return np.array([0, 0, 0, 0, 0, 0, 0])
    if lettre == 1:#'h'
        return np.array([0, 0, 1, 0, 1, 1, 1])
    if lettre == 3:#'l'
        return np.array([0, 0, 0, 1, 1, 1, 0])
    if lettre == 2:#'e'
        return np.array([1, 0, 0, 1, 1, 1, 1])
    if lettre == 4:#'o'
        return np.array([0, 0, 1, 1, 1, 0, 1])
    if lettre == 5:#'w'
        return np.array([0, 1, 1, 1, 1, 1, 1])
    if lettre == 6:#'r'
        return np.array([0, 0, 0, 0, 1, 0, 1])
    if lettre == 7:#'d'
        return np.array([0, 1, 1, 1, 1, 0, 1])
def sortie_memorisee(valeur, afficheur):
...
hello_world = [1, 2, 3, 3, 4, 0, 5, 4, 6, 3, 7, 0]
lettre_a_afficher =[0,0,0,0,0,0]
### Programme
# Initialisation
pygame.init()
...
while True:
    ...
    p = 0
    while p<6:
        sortie_CD4511 = composant_CD4511(sortie_memorisee(lettre_a_afficher[5-p+m],num_afficheur))
        dessiner_arduino(sortie_memorisee(hello_world[5-p+m],num_afficheur), sortie_CD4511, sortie_CD4028(sortie_memorisee(hello_world[5-p+m],num_afficheur)) , sortie_bouton)
        dessiner_afficheur(sortie_CD4511, sortie_CD4028(sortie_memorisee(hello_world[5-p+m],num_afficheur)))
        num_afficheur+=1
        p+=1
    m+=1
    if m>11:
